...
class UNet35tModel(nn.Module):
    def __init__(
        ...
        self.dim_reduction_non_zeros = nn.Sequential(
            conv_nd(dims, 3 * int(conv_ch / 2), conv_ch, 1, padding=0),
            nn.SiLU()
        )
    ...
    def forward(self, x, timesteps, low_res, other=None):
        ...
        com_h = self.SE_Attention_com((1 / 2) * com_h1 + (1 / 2) * com_h2) # + (1 / 3) * com_ht)
        # dist_ht = self.SE_Attention_dist_T1(dist_ht)
        # h = th.cat([com_h, dist_h1, dist_h2, dist_ht], dim=1)
        h = th.cat([com_h, dist_h1, dist_h2], dim=1)
        ...
        return com_h1, com_h2, dist_h1, dist_h2, self.out(h)
    

class SuperResUNet35t(UNet35tModel):
    """
