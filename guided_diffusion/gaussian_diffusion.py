...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        if type(model_output) is tuple:
            if self.b2:
                if not self.t1:
                    (_, _, _, _, _, _, _, _, model_output) = model_output
                else:
                    (_, _, _, _, _, _, _, _, _, _, model_output) = model_output
            else:
                (_, _, _, _, model_output) = model_output
                # (_, _, _, _, _, _, _, _, _, _, _, _, _, _, model_output) = model_output
    ...
    def training_losses(self, model, x_start, t,model_kwargs=None, noise=None, mask=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if self.b2:
                if not self.t1:
                    com_h1, com_h2, com_h3, com_h4, dist_h1, dist_h2, dist_h3, dist_h4, model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
                else:
                    com_h1, com_h2, com_h3, com_h4, com_ht, dist_h1, dist_h2, dist_h3, dist_h4, dist_ht, model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
            else:
                com_h1, com_h2, dist_h1, dist_h2,model_output = model(x_t, self._scale_timesteps(t), **model_kwargs)
                # com_h1, com_h2, com_h3, com_h4, com_h5, com_h6, com_ht, dist_h1, dist_h2, dist_h3, dist_h4, dist_h5, dist_h6, dist_ht, model_output = model(x_t, self._scale_timesteps(t),
                #                                                                         **model_kwargs)
            ...
            terms["mse"] = mean_flat(self.smooth_L1(target, model_output), mask)
            ...
            if self.b2:
                if not self.t1:
                    C = [com_h1, com_h2, com_h3, com_h4]
                    D = [dist_h1, dist_h2, dist_h3, dist_h4]
                else:
                    C = [com_h1, com_h2, com_h3, com_h4, com_ht]
                    D = [dist_h1, dist_h2, dist_h3, dist_h4, dist_ht]
            else:
                C = [com_h1, com_h2]
                D = [dist_h1, dist_h2]
                # C = [com_h1, com_h2, com_h3, com_h4, com_h5, com_h6, com_ht]
                # D = [dist_h1, dist_h2, dist_h3, dist_h4, dist_h5, dist_h6, dist_ht]
            terms['com'] = sum([mean_flat((c[0]-c[1])**2) for c in combinations(C, 2)])
            terms['dist'] = sum([mean_flat((d[0]-d[1])**2) for d in combinations(D, 2)])
